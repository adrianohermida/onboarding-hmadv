...
describe('authenticated route shell smoke', () => {
  ...
  it('keeps every authenticated route mounted inside the shared shell', () => {
    sidebarKeys.forEach(key => {
      ...
      expect(pageHtml).toContain('../js/app.js?v=20260522c');
      expect(pageHtml).toContain('../styles/layout.css?v=20260522c');
      expect(pageHtml).toContain('../styles/components.css?v=20260522c');
